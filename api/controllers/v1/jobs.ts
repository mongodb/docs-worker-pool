...
function getMongoClient(config: IConfig): mongodb.MongoClient {
  const url = `mongodb+srv://${config.get('username')}:${config.get('password')}@${config.get('host')}/?retryWrites=true&w=majority`;
...
async function SubmitArchiveJob(jobId: string) {
  ...
  const response = await new Batch(environment).submitArchiveJob(repo.bucket[environment], `docs-archive-${environment}-mongodb`, repo.prefix[environment]);
