exports = async function(changeEvent) {
    ...
    console.log("this is the status of the job: ", changeEvent.updateDescription.updatedFields)
    ...
    await collection_test.find({ "payload.newHead": commitHash})
      .toArray()
      .then(items => {
        console.log(`Successfully found ${items.length} documents.`);
        items.forEach(function(childJob) {
          //regression_collection.insertOne(childJob);
          if(childJob._id === fullDocument._id){
            return
          }
          console.log(childJob.payload.repoName)
          testJobs[childJob.payload.repoName] = childJob;
          console.log("here ", JSON.stringify(testJobs))
        }
      );
      })
  .catch(err => console.error(`Failed to find documents: ${err}`));
    ...
    await collection_prod.find({ "payload.newHead": commitHash})
      .toArray()
      .then(items => {
        console.log(`Successfully found ${items.length} documents.`);
        items.forEach(function(childJob) {
