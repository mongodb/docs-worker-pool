...
export class ProductionJobHandler extends JobHandler {
  ...
  prepDeployCommands(): void {
    ...
    if (this.currJob.payload.isNextGen) {
      ...
      if (manifestPrefix) {
        this.currJob.deployCommands[
          this.currJob.deployCommands.length - 1
        ] += ` MANIFEST_PREFIX=${manifestPrefix} GLOBAL_SEARCH_FLAG=${this.currJob.payload.stableBranch}`;
      }
    }
  }
  ...
  async constructManifestIndexPath(): Promise<void> {
    try {
      ...
      this.currJob.payload.manifestPrefix =
        output + '-' + (this.currJob.payload.alias ? this.currJob.payload.alias : this.currJob.payload.branchName);
    } catch (error) {
  ...
  async getPathPrefix(): Promise<string> {
    try {
      ...
      if (this.currJob.payload.publishedBranches && this.currJob.payload.publishedBranches.version.active.length > 1) {
        pathPrefix = `${this.currJob.payload.publishedBranches.prefix}/${
          this.currJob.payload.alias ? this.currJob.payload.alias : this.currJob.payload.branchName
        }`;
      } else {
        pathPrefix = `${
          this.currJob.payload.alias ? this.currJob.payload.alias : this.currJob.payload.publishedBranches.prefix
        }`;
      }
      return pathPrefix;
    } catch (error) {
